...
PARAM_GROUPS: Dict[str, Dict[str, Dict[str, Any]]] = {
    "Background Model (GMM)": {
        "gmm_history": {
            ...
            "max": 1000,
            "step": 10,
            "help": "How many past image frames are used to learn what counts as the static background. Higher means more stability, but adaptation to real scene changes becomes slower. 500 frames builds a stable background over ~17s at 30fps. 100 (~3s) adapts fast, which is good for changing light, but noisy early on. 1000 (~33s) is very stable. This is good for fixed outdoor cameras where the scene does not change for minutes.",
        },
        "gmm_var_threshold": {
            ...
            "max": 32.0,
            "step": 0.5,
            "help": "This is the MOG2 variance threshold. It controls how different a pixel must be from the background before it is treated as motion. 8 detects faint motion (pale insects on bright backgrounds) but picks up sensor noise. 16 (default) is balanced. 32 only catches strong contrast changes (dark insect on light background).",
        },
    },
    "Morphological Filtering": {
        "morph_kernel_size": {
            ...
            "max": 7,
            "step": 2,
            "help": "Absolute NxN kernel. Cleans up after motion extraction, this filters noise based on its size and connectivity. 1 is identity (no filtering) - it preserves everything including dead pixels. 3 (default) cleans single-pixel noise. 7 merges small nearby blobs - good for noisy cameras but may erase a 15×15 fruit fly (min_area = 233 px² ≈ 15×15 px). Going above 7 risks eliminating the smallest detectable insects.",
        },
    },
    "Cohesiveness": {
        "min_largest_blob_ratio": {
            ...
            "max": 0.95,
            "step": 0.01,
            "help": "Minimum ratio of largest color blob to total motion area in detection region. If the largest blob is smaller than this fraction of total motion, it's scattered (plant/wind). 0.50 accepts detections where half the motion is one blob - catches wing-separated insects but allows false positives. 0.80 (default) is balanced. 0.95 demands near-uniform motion - rejects anything with separated wings or motion blur.",
        },
        "max_num_blobs": {
            ...
            "max": 10,
            "step": 1,
            "help": "Upper limit on disconnected moving regions inside one candidate area. Lower values are stricter and reduce clutter-induced detections. 1 requires a single contiguous blob - strict, rejects insects with separated wings. 5 (default) tolerates 2–3 wing segments + legs. 10 is very permissive - likely catches wind-blown debris.",
        },
        "min_motion_ratio": {
            ...
            "max": 0.30,
            "step": 0.01,
            "help": "Fraction of bbox filled with motion pixels. Higher values require denser, more convincing motion; lower values accept sparse movement. 0.05 keeps very sparse detections - catches slow insects that only partially move per frame. 0.15 (default) is balanced. 0.30 requires dense filling - rejects motion blur or partially occluded insects.",
        },
    },
    "Shape": {
        "min_area": {
            ...
            "max": 500,
            "step": 1,
            "help": "Smallest allowed detection area in pixels squared. Raise this to ignore dust-like or sensor-noise artifacts. Resolved from 0.0002 × 1080² for a 1080×1080 frame. 50 px² (~7×7) catches fruit flies (≈10×10 px) but risks single-pixel noise. 233 (~15×15, default) filters below a housefly - safe for most use cases. 500 (~22×22) only passes butterflies/moths - rejects small flies. The 233 default is calibrated: it's ~0.02% of the frame, small enough for real insects but large enough to kill noise.",
        },
        "max_area": {
            ...
            "max": 100000,
            "step": 10,
            "help": "Largest allowed detection area in pixels squared. Lower this to reject birds, branches, or large shadows. Resolved from 0.035 × 1080² for a 1080×1080 frame. 10,000 (~100×100) rejects objects larger than 100×100 - good for small-insect-only detection. 40,824 (~202×202, default) correctly filters birds (~50,000 px²). 100,000 (~316×316) accepts anything up to ~316×316 - allows birds/large debris.",
        },
        "min_density": {
            ...
            "max": 5.0,
            "step": 0.1,
            "help": "How compact the contour should be, based on area relative to boundary length (area/perimeter ratio). A circle of diameter d has density d/4. So 1.5 passes objects with effective diameter ≥ 6 px (very elongated shapes). 3.0 (default) requires diameter ≥ 12 px (compact blobs). 5.0 requires diameter ≥ 20 px (nearly circular). For insects, 3.0–5.0 catches round-ish bodies while rejecting linear artifacts like grass blades.",
        },
        "min_solidity": {
            ...
            "max": 0.80,
            "step": 0.01,
            "help": "How fully the contour fills its own convex envelope (convex hull fill ratio). Higher values prefer smoother and less ragged shapes. 0.30 tolerates very irregular outlines - catches moths with ragged wings. 0.55 (default) is solid enough to reject scattered motion. 0.80 demands smooth convex shapes - rejects insects with protruding legs/antennae. Insects typically have solidity 0.5–0.8 depending on posture.",
        },
    },
    "Tracking": {
        "min_displacement": {
            ...
            "max": 100.0,
            "step": 1.0,
            "help": "Minimum net distance a track must travel to be accepted as real movement. Increase this to suppress stationary false positives. Resolved from 0.05 × 1080 for a 1080px-wide frame. 20 px (~2% of width) confirms insects that barely move (2–3 frames of a slow crawler). 54 (~5%, default) requires crossing ~5% of the frame - filters stationary false positives. 100 (~9%) only confirms insects that cross ~10% of the frame - good for 'detect insects traversing the scene' but misses brief visitors.",
        },
        "min_path_points": {
            ...
            "max": 25,
            "step": 1,
            "help": "Minimum number of time points required before movement-pattern tests are trusted. 3 confirms after just 3 frames (0.1s) - catches very brief sightings but risks noise. 10 (default) requires ~0.3s of tracking - a good balance. 25 requires ~0.8s - only persistent insects. Note: this interacts with insect speed - a fast butterfly at 50 px/frame crosses the frame in 22 frames, so a value of 25 would require it to be visible for the entire crossing.",
        },
        "max_frame_jump": {
            ...
            "max": 200.0,
            "step": 1.0,
            "help": "Largest position jump allowed between two consecutive image frames for the same track. Lower values prevent accidental links between different objects. Resolved from 0.1 × 1080 for a 1080px-wide frame. At 30fps: a slow beetle moves ~5 px/frame, a fly ~20 px/frame, a butterfly ~50 px/frame, a dragonfly ~80 px/frame. 40 keeps up with flies but loses fast butterflies. 108 (default) handles all common insects. 200 is very permissive - risks matching wrong detections across frames.",
        },
        "max_lost_frames": {
            ...
            "max": 90,
            "step": 1,
            "help": "Number of frames a track can disappear before deletion. Higher values help reconnect after occlusion; too high can fuse unrelated trajectories. 10 (~0.3s) drops lost tracks quickly - no recovery from brief occlusions. 45 (~1.5s, default) recovers from a brief occlusion - good for plants briefly covering an insect. 90 (~3s) keeps searching for longer - useful for insects that hide behind objects, but longer values increase risk of false re-association.",
        },
        "max_area_change_ratio": {
            ...
            "max": 5.0,
            "step": 0.1,
            "help": "Largest multiplicative size change allowed from one frame to the next for a consistent track. 1.5× requires very stable size - rejects perspective changes or wing flapping. 3.0× (default) allows moderate size variation - handles wing beats (area can 2× during a flap cycle). 5.0× is very permissive - allows insects to approach the camera.",
        },
    },
    "Tracker Matching": {
        "tracker_w_dist": {
            ...
            "max": 0.9,
            "step": 0.01,
            "help": "Higher = prioritise spatial proximity when linking detections between frames. 0.3 lets size similarity dominate - good when insects are similar-sized and close together. 0.6 (default) is balanced. 0.9 is almost purely position-based - good when insects vary greatly in size. Must sum to 1.0 with the size weight.",
        },
        "tracker_w_area": {
            ...
            "max": 0.7,
            "step": 0.01,
            "help": "Complement of the position weight - importance of size similarity during matching. 0.1 ignores size - only distance matters. 0.4 (default) is moderate. 0.7 strongly prefers same-sized detections - good for tracking a single individual. Must sum to 1.0 with the position weight.",
        },
        "tracker_cost_threshold": {
            ...
            "max": 0.5,
            "step": 0.01,
            "help": "Maximum acceptable mismatch (cost) score when linking detections to existing tracks. 0.1 is very strict - only near-identical detections match (risk: more orphan tracks). 0.3 (default) is balanced. 0.5 is lenient -  more matches but risk of ID switches when insects cross paths.",
        },
    },
    "Path Topology": {
        "max_revisit_ratio": {
            ...
            "max": 0.80,
            "step": 0.01,
            "help": "Maximum fraction of the path's point-pairs that are close together (revisiting earlier locations). Lower values require more exploratory movement. A straight-line path at 5 px/frame spacing gives a ratio of ~0.09 (passes easily); a slow crawler at 1 px/frame spacing gives ~0.99 (fails). 0.10 only accepts very linear paths. 0.30 (default) allows some local oscillation. 0.80 tolerates nearly-stationary behaviour - almost everything passes.",
        },
        "min_progression_ratio": {
            ...
            "max": 0.95,
            "step": 0.01,
            "help": "Minimum proportion of movement that advances rather than oscillates locally (net_displacement / max_distance_from_start). 0.30 allows heavy backtracking - catches insects that meander. 0.70 (default) requires clear forward movement. 0.95 demands a near-linear trajectory - only straight-line fliers pass.",
        },
        "max_directional_variance": {
            ...
            "max": 1.00,
            "step": 0.01,
            "help": "How much direction changes are tolerated along a track. 0 is perfectly straight, 1 is a random walk. 0.30 requires consistent heading (straight-line fliers). 0.90 (default) allows significant direction changes (insects that turn). 1.00 accepts random movement - effectively disabled.",
        },
        "revisit_radius": {
            ...
            "max": 100.0,
            "step": 1.0,
            "help": "Spatial radius in pixels used to decide if a point is considered a return to a previously visited area. Resolved from 0.05 × 1080 for a 1080px-wide frame. 20 px (~2%) only flags very close points - generous to slow crawlers. 54 px (~5%, default) catches moderate oscillation. 100 px (~9%) is aggressive - even a slow insect moving in a gentle curve gets flagged. Key interaction: with a revisit ratio of 0.30 and this at 54, the system passes insects moving at roughly 15 px/frame or faster on straight lines.",
        },
    },
}
